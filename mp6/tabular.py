...
class TabQPolicy(QPolicy):
    ...
    def qvals(self, states):
        ...
        """
        qvals = np.zeros((len(states), 2))
        for i in range(len(states)):
            discretized_state = self.discretize(states[i])
            qvals[i][0] = self.model[discretized_state][0]
            qvals[i][1] = self.model[discretized_state][1]
        return qvals
    ...
    def td_step(self, state, action, reward, next_state, done):
        ...
        """
        discretized_cur_state = self.discretize(state)
        q_values = self.model[discretized_cur_state][action]
        discretized_next_state = self.discretize(next_state)
        
        if done is False:
            target = reward + self.gamma * np.max(self.model[discretized_next_state])
        else:
            target = reward

        self.model[discretized_cur_state][action] += self.lr * (target - q_values)
